...
const (
	...
	TypeOpenStackChaos TemplateType = "OpenStackChaos"
...
type EmbedChaos struct {
	...
	// +optional
	OpenStackChaos *OpenStackChaosSpec `json:"openstackChaos,omitempty"`
	// +optional
...
func (it *EmbedChaos) SpawnNewObject(templateType TemplateType) (GenericChaos, error) {
	...
	case TypeNetworkChaos:
		...
		return &result, nil
	case TypeOpenStackChaos:
		result := OpenStackChaos{}
		result.Spec = *it.OpenStackChaos
		return &result, nil
...
func (in *OpenStackChaosList) GetItems() []GenericChaos {
	var result []GenericChaos
	for _, item := range in.Items {
		item := item
		result = append(result, &item)
